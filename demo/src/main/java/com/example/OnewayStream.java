...
public class OnewayStream implements Runnable {
  ...
  private OnewayTrackFrame trackView;

  private Thread micThread;

  public OnewayStream(String languageCode, OnewayTrackFrame trackView) {
    this.languageCode = languageCode;
    this.trackView = trackView;
    this.direction = "none";
  }

  public OnewayStream(String languageCode, OnewayTrackFrame trackView, String dir) {
    this.languageCode = languageCode;
    this.trackView = trackView;
    this.direction = dir;
  }
  ...
  public void infiniteStreamingRecognize(String languageCode, final OnewayTrackFrame trackView) throws Exception {
          ...
          if (result.getIsFinal()) {
            ...
            try { // 입력 종료 시 출력
              ...
              if (direction == "none" || direction == "left") {
                trackView.leftTrackPanel.setText(alternative.getTranscript());
                req.put("text", trackView.leftTrackPanel.getText());
                trackView.addLine(trackView.leftTrackPanel.getText(), httpsTranslator.translateRequest(req), Color.GRAY, Color.WHITE);
                trackView.leftTrackPanel.clearText();
              // 오른쪽 페이지 설정
              } else {
                trackView.rightTrackPanel.setText(alternative.getTranscript());
                req.put("text", trackView.rightTrackPanel.getText());
                trackView.addLine(httpsTranslator.translateRequest(req), trackView.rightTrackPanel.getText(), Color.WHITE, Color.GRAY);
                trackView.rightTrackPanel.clearText();
              }
            } catch (Exception e) {
              e.printStackTrace();
            }
